...
const EmailForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const validEmails = formData.recipients.filter(email => email.trim() !== '');
    
    if (validEmails.length === 0) {
      setToast({ 
        message: 'Veuillez saisir au moins un destinataire', 
        type: 'error' 
      });
      return;
    }
    ...
    try {
      const payload = {
        to: validEmails, // Envoyer un tableau d'emails
      ...
      setToast({ 
        message: response.data.message || `Email envoyé avec succès à ${validEmails.length} destinataire(s)!`, 
        type: 'success' 
      });
    ...
    <div className="card max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">Ajoutez plusieurs destinataires si nécessaire</p>
        </div>
        ...
        <div className="text-sm text-gray-500">
          <p>Destinataires: {formData.recipients.filter(e => e.trim() !== '').length}</p>
        </div>
